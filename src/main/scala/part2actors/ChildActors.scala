...
object ChildActors extends App {
  ...
  // actor hierarchies
  // parent -> child
  // who own Parent actor? --- Guardian actor

  /*
    Guardian Actor(top-level)
      - /system = system guardian
      - /user   = user-level guardian (created using system.actorOf)
      - / = Root guardian (it's children are: /system & /user)
   */

  /**
    * Finding actor by path - Actor Selection
    */
  ...
  class CreditCard extends Actor {
    ...
    def attachedTo(account: ChildActors.NaiveBankAccount): Receive = {
      case CheckStatus =>
        ...
        account.withdraw(1)   // since, we passed object in NaiveBankAccount, we can directly withdraw
    }
  }
  ...
  bankAccountRef ! InitializedAccount

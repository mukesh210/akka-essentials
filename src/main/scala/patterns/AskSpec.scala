...
class AskSpec extends TestKit(ActorSystem("AskSpec"))
  ...
  def authenticatorTestSuite(props: Props) = {
    "fail to authenticate a non-registered user" in {
      ...
      authManager ! Authenticate("Mukesh", "rtjvm")
    ...
    "fail to authenticate if invalid password" in {
      val authManager = system.actorOf(props)
      authManager ! RegisterUser("Mukesh", "rtjvm")
      authManager ! Authenticate("Mukesh", "iloveakka")
...
object AskSpec {
  ...
  class KVActor extends Actor with ActorLogging {
      ...
      def online(kv: Map[String, String]): Receive = {
        case Read(key) =>
          log.info(s"Trying to read value at key: ${key}")
          sender() ! kv.get(key)
  ...
  class AuthManager extends Actor with ActorLogging {
    import AuthManager._
    val authDb = context.actorOf(Props[KVActor], "kvActor")
    //step 2: logistics
    ...
    def handleAuthenticate(username: String, password: String) = {
